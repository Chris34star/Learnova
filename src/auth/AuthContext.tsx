...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const load=useCallback(async(next:User|null)=>{setUser(next);setProfileError(null);if(!next){setProfile(null);setSchool(null);setLoading(false);return}try{const result=await authService.profile(next.id);setProfile(result.profile);setSchool(result.school);if(!result.profile)setProfileError('Your account is authenticated but has not been provisioned. Contact a Learnova administrator.')}catch{setProfile(null);setSchool(null);setProfileError('We could not load your account. Try signing in again or contact your administrator.')}finally{setLoading(false)}},[]);
